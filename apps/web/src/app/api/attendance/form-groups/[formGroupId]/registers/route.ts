...
export async function POST(
  request: NextRequest,
  { params }: { params: Promise<{ formGroupId: string }> },
) {
  const { formGroupId } = await params;
  const body = await request.json();
  const result = await fetchApiForCurrentUser(
    `/attendance/form-groups/${formGroupId}/registers`,
    {
      method: "POST",
